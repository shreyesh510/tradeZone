...
interface EditTradePnLModalProps {
  isOpen: boolean;
  onClose: () => void;
  isDarkMode: boolean;
  item: any;
...
const EditTradePnLModal: React.FC<EditTradePnLModalProps> = ({ isOpen, onClose, isDarkMode, item, onSave }) => {
  ...
  useEffect(() => {
    if (item) {
      setFormData({
        netPnL: item.netPnL?.toString() || '',
        totalTrades: item.totalTrades?.toString() || '',
        notes: item.notes || ''
      });
    }
  }, [item]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      loss,
      netPnL,
      totalTrades: parseInt(formData.totalTrades) || undefined,
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className={`${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'} w-full max-w-md rounded-2xl shadow-xl overflow-hidden`}>
        <div className={`px-6 py-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <h3 className="text-lg font-semibold">Edit Trade P&L Record</h3>
...
export default EditTradePnLModal;